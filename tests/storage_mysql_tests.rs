//! MySQL CRUD integration test stubs for StorageSqlx<MySql>.
//!
//! These tests require a running MySQL instance and the `MYSQL_DATABASE_URL`
//! environment variable to be set (e.g., `mysql://user:pass@localhost/test_db`).
//!
//! Tests are gated with `#[cfg(feature = "mysql")]` and marked `#[ignore]`
//! so they do not run by default.
//!
//! Run with: cargo test --features mysql -- --ignored --test-threads=1
...
mod storage_mysql {
    ...
    use bsv_wallet_toolbox::storage::sqlx_impl::MysqlStorage;
    ...
    /// Helper to create a MySQL storage connected to the test database.
    ///
    /// Reads `MYSQL_DATABASE_URL` from the environment, creates the storage,
    /// runs migrations, and drops all existing data for a clean slate.
    async fn setup_mysql_storage() -> WalletResult<MysqlStorage> {
        let url = std::env::var("MYSQL_DATABASE_URL")
            .expect("MYSQL_DATABASE_URL must be set to run MySQL tests");
        let config = StorageConfig {
            url,
            ..Default::default()
        };
        let storage = MysqlStorage::new_mysql(config, Chain::Test).await?;
    ...
    async fn insert_mysql_user(storage: &MysqlStorage, identity_key: &str) -> i64 {
    ...
    async fn insert_mysql_transaction(
    ...
    async fn setup_mysql_update_fixture() -> (MysqlStorage, UpdateFixture) {
        let storage = setup_mysql_storage().await.unwrap();
        let settings = storage.make_available().await.unwrap();
        let now = test_datetime();
        let user_id = insert_mysql_user(&storage, "mysql-update-user").await;
        ...
        let certificate_id = storage
            .insert_certificate(
                &Certificate {
                    ...
                    serial_number: "mysql-update-certificate".to_string(),
        ...
        let basket_id = storage
            ...
            .await
            .unwrap();
        let output_id = insert_mysql_output(
            &storage,
            user_id,
            transaction_id,
            0,
        ...
        let proven_tx_id = storage
            .insert_proven_tx(
                &ProvenTx {
                    ...
                    updated_at: now,
                    proven_tx_id: 0,
                    txid: "mysql-update-proven-tx".to_string(),
                    height: 800_000,
                    index: 1,
        ...
        let proven_tx_req_id = storage
            .insert_proven_tx_req(
                &ProvenTxReq {
                    ...
                    attempts: 0,
                    notified: false,
                    txid: "mysql-update-proven-tx-req".to_string(),
                    batch: None,
                    history: "{}".to_string(),
                    notify: "{}".to_string(),
                    raw_tx: vec![1, 2, 3],
        ...
        let sync_state_id = storage
            .insert_sync_state(
                &SyncState {
                    ...
                    sync_state_id: 0,
                    user_id,
                    storage_identity_key: "mysql-update-remote-key".to_string(),
                    storage_name: "remote".to_string(),
                    status: SyncStatus::Unknown,
                    init: false,
                    ref_num: "mysql-update-sync-state".to_string(),
                    sync_map: "{}".to_string(),
                    when: None,
                    satoshis: None,
                    error_local: None,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_insert_and_find_user() {
        let storage = setup_mysql_storage().await.unwrap();
        let now = test_datetime();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_insert_and_find_transaction() {
        let storage = setup_mysql_storage().await.unwrap();
        let now = test_datetime();
        ...
        let tx = Transaction {
            ...
            reference: "ref-mysql-001".to_string(),
            is_outgoing: true,
            satoshis: 50000,
            description: "MySQL test transaction".to_string(),
            version: Some(1),
            lock_time: Some(0),
        ...
        assert_eq!(results[0].reference, "ref-mysql-001");
    }
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_insert_and_count_output_basket() {
        let storage = setup_mysql_storage().await.unwrap();
        let now = test_datetime();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_no_script_projection() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-no-script-user").await;
        let tx_id = insert_mysql_transaction(
            &storage,
            user_id,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_users_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        insert_mysql_user(&storage, "mysql-count-users").await;
        let unpaged = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_certificates_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-certificates").await;
        storage
            .insert_certificate(
                &Certificate {
                    ...
                    user_id,
                    cert_type: "identity".to_string(),
                    serial_number: "mysql-count-certificates".to_string(),
                    certifier: "certifier".to_string(),
                    subject: "subject".to_string(),
                    verifier: None,
                    revocation_outpoint: "outpoint.0".to_string(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_certificate_fields_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-certificate-fields").await;
        let certificate_id = storage
            .insert_certificate(
                &Certificate {
                    created_at: test_datetime(),
                    ...
                    user_id,
                    cert_type: "identity".to_string(),
                    serial_number: "mysql-count-certificate-fields".to_string(),
                    certifier: "certifier".to_string(),
                    subject: "subject".to_string(),
                    verifier: None,
                    revocation_outpoint: "outpoint.0".to_string(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_commissions_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-commissions").await;
        let transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-count-commissions",
        )
        .await;
        storage
            .insert_commission(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_monitor_events_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        storage
            .insert_monitor_event(
                &MonitorEvent {
                    ...
                    updated_at: test_datetime(),
                    id: 0,
                    event: "mysql-count-monitor-events".to_string(),
                    details: Some("{}".to_string()),
                },
                None,
            )
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_output_baskets_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-output-baskets").await;
        storage
            .insert_output_basket(
                &OutputBasket {
                    created_at: test_datetime(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_output_tag_maps_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-output-tag-maps").await;
        let transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-count-output-tag-maps",
        )
        .await;
        let output_id =
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_output_tags_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-output-tags").await;
        storage
            .insert_output_tag(
                &OutputTag {
                    created_at: test_datetime(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_outputs_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-outputs").await;
        let transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_proven_txs_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        storage
            .insert_proven_tx(
                &ProvenTx {
                    ...
                    updated_at: test_datetime(),
                    proven_tx_id: 0,
                    txid: "mysql-count-proven-txs".to_string(),
                    height: 800_000,
                    index: 1,
                    merkle_path: vec![1, 2, 3],
                    raw_tx: vec![4, 5, 6],
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_proven_tx_reqs_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        storage
            .insert_proven_tx_req(
                &ProvenTxReq {
                    ...
                    attempts: 0,
                    notified: false,
                    txid: "mysql-count-proven-tx-reqs".to_string(),
                    batch: None,
                    history: "{}".to_string(),
                    notify: "{}".to_string(),
                    raw_tx: vec![1, 2, 3],
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_settings_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_sync_states_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-sync-states").await;
        storage
            .insert_sync_state(
                &SyncState {
                    created_at: test_datetime(),
                    ...
                    status: SyncStatus::Unknown,
                    init: false,
                    ref_num: "mysql-count-sync-states".to_string(),
                    sync_map: "{}".to_string(),
                    when: None,
                    satoshis: None,
                    error_local: None,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_transactions_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-transactions").await;
        insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-count-transactions",
        )
        .await;
        let unpaged = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_tx_label_maps_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-tx-label-maps").await;
        let transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-count-tx-label-maps",
        )
        .await;
        let tx_label_id = storage
            .insert_tx_label(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_count_tx_labels_with_paged_args() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-count-tx-labels").await;
        storage
            .insert_tx_label(
                &TxLabel {
                    created_at: test_datetime(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_non_empty_transaction_status_list() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-output-status-list").await;
        ...
        .enumerate()
        {
            let transaction_id = insert_mysql_transaction(
                ...
                user_id,
                status,
                &format!("mysql-output-status-{index}"),
            )
            .await;
            let output_id = insert_mysql_output(
                &storage,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_empty_transaction_status_list_result() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-empty-output-status-list").await;
        let transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-empty-output-status-list",
        )
        .await;
        insert_mysql_output(&storage, user_id, transaction_id, 0, true, None, None).await;
        ...
        let result = storage
            ...
            .await;
        println!("MySQL empty tx_status IN-list result: {result:?}");
        assert!(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_combined_filters() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-combined-output-user").await;
        let other_user_id = insert_mysql_user(&storage, "mysql-combined-output-other").await;
        let completed_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-combined-completed",
        )
        .await;
        let unproven_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Unproven,
            "mysql-combined-unproven",
        )
        .await;
        let other_tx_id = insert_mysql_transaction(
            &storage,
            other_user_id,
            TransactionStatus::Completed,
            "mysql-combined-other",
        )
        .await;
        let expected_id = insert_mysql_output(
            &storage,
            user_id,
            completed_id,
            0,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_limit_offset_two_pages() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-paged-outputs").await;
        let transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
        ...
        for vout in 0..4 {
            expected.insert(
                insert_mysql_output(&storage, user_id, transaction_id, vout, true, None, None)
                    .await,
            );
        }
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_zero_limit_result() {
        let storage = setup_mysql_storage().await.unwrap();
        ...
        let result = storage
            ...
            .await;
        println!("MySQL LIMIT 0 result: {result:?}");
        assert!(
            result.is_ok() || result.is_err(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_outputs_negative_limit_result() {
        let storage = setup_mysql_storage().await.unwrap();
        ...
        let result = storage
            ...
            .await;
        println!("MySQL LIMIT -1 result: {result:?}");
        assert!(
            result.is_ok() || result.is_err(),
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_list_actions_with_labels_inputs_outputs_and_scripts() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-list-actions-user").await;
        let source_tx_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-list-actions-source",
        )
        .await;
        let action_tx_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-list-actions-action",
        )
        .await;
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_delete_monitor_events_before_id_scopes_event_and_boundary() {
        let storage = setup_mysql_storage().await.unwrap();
        ...
        // Probes: the shared server DELETE must use MySQL placeholders and
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_transactions_limit_offset_two_pages() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-paged-transactions").await;
        let mut expected = BTreeSet::new();
        for index in 0..4 {
            expected.insert(
                insert_mysql_transaction(
                    ...
                    user_id,
                    TransactionStatus::Completed,
                    &format!("mysql-paged-transaction-{index}"),
                )
                .await,
            );
        }
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_find_users_limit_offset_two_pages() {
        let storage = setup_mysql_storage().await.unwrap();
        let mut expected = BTreeSet::new();
        for index in 0..4 {
            expected
                .insert(insert_mysql_user(&storage, &format!("mysql-paged-user-{index}")).await);
        }
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_user_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_user(
                fixture.user_id,
                &UserPartial {
                    identity_key: Some("mysql-updated-user".to_string()),
        ...
        assert_eq!(rows[0].identity_key, "mysql-updated-user");
        assert_eq!(rows[0].updated_at, updated_at);
    }

    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_certificate_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_certificate(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_certificate_field_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_commission_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_commission(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_monitor_event_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_output_basket_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_output_tag_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_output_tag(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_output_tag_map_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_output_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_output(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_mark_inputs_spent_claims_only_eligible_rows_once() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-mark-inputs-user").await;
        let other_user_id = insert_mysql_user(&storage, "mysql-mark-inputs-other-user").await;
        let source_transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-mark-inputs-source",
        )
        .await;
        let other_source_transaction_id = insert_mysql_transaction(
            &storage,
            other_user_id,
            TransactionStatus::Completed,
            "mysql-mark-inputs-other-source",
        )
        .await;
        let previous_spender_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-mark-inputs-previous",
        )
        .await;
        let spending_transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-mark-inputs-spending",
        )
        .await;
        let eligible_one = insert_mysql_output(
            &storage,
            user_id,
        ...
        .await;
        let eligible_two = insert_mysql_output(
            &storage,
            user_id,
            source_transaction_id,
            1,
        ...
        .await;
        let already_spent = insert_mysql_output(
            &storage,
            user_id,
            source_transaction_id,
            2,
        ...
        .await;
        let other_users_output = insert_mysql_output(
            &storage,
            other_user_id,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_release_inputs_only_releases_matching_claimant() {
        let storage = setup_mysql_storage().await.unwrap();
        let user_id = insert_mysql_user(&storage, "mysql-release-inputs-user").await;
        let source_transaction_id = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Completed,
            "mysql-release-inputs-source",
        )
        .await;
        let transaction_a = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Failed,
            "mysql-release-inputs-a",
        )
        .await;
        let transaction_b = insert_mysql_transaction(
            &storage,
            user_id,
            TransactionStatus::Unsigned,
            "mysql-release-inputs-b",
        )
        .await;
        let claimed_by_a = insert_mysql_output(
            &storage,
            user_id,
            source_transaction_id,
            0,
        ...
        .await;
        let claimed_by_b = insert_mysql_output(
            &storage,
            user_id,
            source_transaction_id,
            1,
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_proven_tx_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_proven_tx(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_proven_tx_req_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_settings_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_settings(
                &SettingsPartial {
                    storage_name: Some("mysql-updated-storage".to_string()),
                    updated_at: Some(updated_at),
                    ..Default::default()
                },
                None,
        ...
        let rows = storage
            ...
            .unwrap();
        assert_eq!(rows.len(), 1);
        assert_eq!(rows[0].storage_name, "mysql-updated-storage");
        assert_eq!(rows[0].updated_at, updated_at);
    }

    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_transaction_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_transaction(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_tx_label_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_tx_label(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_tx_label_map_with_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let affected = storage
            .update_tx_label_map(
    ...
    #[tokio::test]
    #[ignore]
    async fn test_mysql_update_sync_state_with_when_and_explicit_updated_at() {
        let (storage, fixture) = setup_mysql_update_fixture().await;
        let updated_at = update_datetime();
        let when =
